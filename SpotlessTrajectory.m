classdef SpotlessTrajectory < handle
    ...
    methods
        function self = SpotlessTrajectory(robot, plates, gripper)
        ...
        function Run(self)
            ...
            for plateIndex = 1:9
                self.AbovePickupPoint();  % Move to the point above the plate's pickup position
                self.PickupPlate(plateIndex);  % Move to and pick up the plate
                self.AbovePickupPoint();  % Return to above the pickup point
                self.AboveDropoffZone();  % Move to the point above the drop-off zone
                self.ReleasePlate(plateIndex);  % Drop the plate in the designated location
            end
        ...
        function PickupPlate(self, plateIndex)
            ...
            t2 = transl(self.plates.platesStart(plateIndex,:)) * rpy2tr(180,0,0,'deg');
        ...
        function ReleasePlate(self, plateIndex)
            ...
            % of the plate
            t4 = transl(self.plates.platesEnd(plateIndex,:)) * rpy2tr(180,0,0,'deg');
