classdef SpotlessTrajectoryRMRC < handle
    ...
    methods
        function self = SpotlessTrajectoryRMRC(robot2, robot1, plates, gripper, app)
            ...
            self.stepsSixteen = 50;  % Set the number of steps for trajectory interpolation
            self.stepsThree = 50;
            self.qStartSixteen = [0 -135*pi/180 pi/4 pi/2 0 0];  % Initial joint configuration
            ...
            %
            % self.robot2 = robot2;  % Assign the robot instance
            % self.stepsThree = 10;  % Set the number of steps for trajectory interpolation
        ...
        function InfrontOfTap(self)
            ...
            for j = 1:self.stepsSixteen
                ...
                if self.eStopFlag == 1
                     ...
                     % currentstep = self.robot1.model.fkine(self.qEndSixteen).T;
                     self.robot1.model.animate(currentstep);
                end
        ...
        function PickRotatedUp(self, plateIndex)
            ...
            for j = 1:self.stepsSixteen
                self.robot1.model.animate(path2(j, :));  % Animate the robot at each step
        ...
        function AboveDropPoint(self)
            ...
            % Compute the actual joint state using forward kinematics after inverse kinematics
            self.tActualSixteen = self.robot1.model.fkine(self.qEndSixteen).T;  % Get the actual transformation matrix
            self.desiredPoseSixteen = t3(1:3, 4);  % Extract the desired position (x, y, z) from transformation matrix
            self.actualPoseSixteen = self.tActualSixteen(1:3, 4);  % Extract the actual position from the forward kinematics result
            ...
            % Animate the robot and gripper along the trajectory
            for j = 1:self.stepsSixteen
                self.robot1.model.animate(path3(j, :));  % Animate the robot at each step
                % Tr = self.robot1.model.fkine(path3(j,:)).T * troty(-pi/2);  % Get the transformation matrix and rotate the gripper
                % self.gripper.robot.delay = 0;  % No delay for gripper animation
                % self.gripper.animateGripper(Tr);  % Animate the gripper to follow the end-effector
                axis equal;  % Keep axis scaling equal for proper visualization
                drawnow();  % Update the figure window for real-time animation
                ...
                % 
                % % Compute and display the final transformation at the last step
                % Tr3 = self.robot1.model.fkine(path3(size(self.stepsSixteen,1),:)).T;  % Get the final transformation matrix
        ...
        function Home(self)
            ...
            for j = 1:self.stepsSixteen
                self.robot1.model.animate(path5(j, :));  % Animate the robot joint states
        ...
        function POC1(self)
            ...
            T1 = transl([-0.8 0.7800 1.8]) * rpy2tr(180, 0, 0, 'deg');      %initial translation
            T2 = transl([-0.8 0.7800 1.45]) * rpy2tr(180,0,0,'deg');



% 3.5: Resolved Motion Rate Control V1
...
% 3.6
x2 = [T2(1,4) T2(2,4) T2(3,4) 1 1 1]';                      %same as qStart
x1 = [0.3 -0.4 0.3 1 1 1]';
deltaT = 1000;                                        % Discrete time step
qMatrix = nan(self.stepsThree,6);
% 3.7
x = zeros(6,self.stepsThree);
s = lspb(0,1,self.stepsThree);                                 % Create interpolation scalar
for i = 1:self.stepsThree
...
% T1 = transl(-0.8, 0.78, 1.8) * rpy2tr(180, 0, 0, 'deg'), 'q0', self.qStartThree, 'mask', [1,1,1,0,0,0];
% qMatrix(1,:) = self.robot2.model.ikine(T1);                 % Solve for joint angles       %T1 is just translation and rotation of initial waypoint

qMatrix(1,:) = self.robot2.model.ikcon(transl([-0.8 0.7800 1.8]) * rpy2tr(180,0,0,'deg'));


% 3.10
for i = 1:self.stepsThree-1
    xdot = (x(:,i+1) - x(:,i))/deltaT;                             % Calculate velocity at discrete time step
    J = self.robot2.model.jacob0(qMatrix(i,:));            % Get the Jacobian at the current state
    J = J(1:6,:);                           % Take only first 2 rows
    qdot = inv(J)*xdot;                             % Solve velocitities via RMRC
    qMatrix(i+1,:) =  qMatrix(i,:) + deltaT*qdot';                   % Update next joint state
end
               self.robot2.model.animate(qMatrix(i+1,:));
...
% % 3.5: Resolved Motion Rate Control V2 %%%%%%%%%%%%%%%%%
% 
% steps_per_segment = 50;
% % Define multiple waypoints (positions)
% waypoints = [
%     -0.8, 0.78, 1.8;    % Start position
%     -0.8 0.7800 1.45;    % Intermediate waypoint 1
%     -0.9 0.7800 1.45;
%     -0.7 0.7800 1.45       % End position
% ]';
% 
% deltaT = 0.05;                                        % Discrete time step
% 
% % % 3.7
% % x = zeros(3,self.stepsThree);
% % s = lspb(0,1,self.stepsThree);                                 % Create interpolation scalar
% % for i = 1:self.stepsThree
% %     x(:,i) = x1*(1-s(i)) + s(i)*x2;                  % Create trajectory in x-y plane
% % end
% 
% % Define orientations (assuming fixed orientation here)
% orientation = rpy2tr(180, 0, 0, 'deg');
% 
...
% % its argument name. E.g. Initial Q Guess = 'q0', q & Mask = 'mask', m.
% 
% % T1 = transl(-0.8, 0.78, 1.8) * rpy2tr(180, 0, 0, 'deg'), 'q0', self.qStartThree, 'mask', [0,0,0,0,0,0];
% % qMatrix(1,:) = self.robot2.model.ikine(T1);                 % Solve for joint angles       %T1 is just translation and rotation of initial waypoint
% 
% % Loop over each segment between waypoints
% for w = 1:size(waypoints, 2) - 1
%     % Set up start and end points for the current segment
%     x1 = waypoints(:, w);
%     x2 = waypoints(:, w+1);
% 
%     % Interpolate positions for this segment
%     x = zeros(3, steps_per_segment);
%     s = lspb(0, 1, steps_per_segment); % Linear scalar for smooth interpolation
%     for i = 1:steps_per_segment
%         x(:, i) = x1 * (1 - s(i)) + s(i) * x2;
%     end
% 
%     % Initialize segment qMatrix
%     qSegment = nan(steps_per_segment, 6);
% 
%     % Solve for initial joint angles using inverse kinematics
%     T1 = transl(x1') * orientation;
%     qSegment(1, :) = self.robot2.model.ikcon(T1);
% 
%     % Define fixed angular velocity (optional; can vary per segment)
%     omega = [0; 0; 0];  % No rotation for this example
% 
%     % Compute joint trajectory for each step in the segment
%     for i = 1:steps_per_segment-1
%         % Linear velocity between points
%         xdot_linear = (x(:, i+1) - x(:, i)) / deltaT;
% 
%         % Combine linear and angular velocities
%         xdot = [xdot_linear; omega];
% 
%         % Compute joint velocities using Jacobian
%         J = self.robot2.model.jacob0(qSegment(i, :)); % Full 6x6 Jacobian
%         qdot = pinv(J) * xdot;  % Use pseudo-inverse for stability
% 
%         % Update joint positions
%         qSegment(i+1, :) = qSegment(i, :) + deltaT * qdot';
%     end
% 
%     % Append segment to full trajectory
%     qMatrix = [qMatrix; qSegment];
% end
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


% qMatrix(1,:) = self.robot2.model.ikcon(transl([-0.8 0.7800 1.45]) * rpy2tr(180,0,0,'deg'));
% 
% omega = [0; 0; 0];
% 
% % 3.10
% for i = 1:self.stepsThree-1
%     xdot_linear = (x(:,i+1) - x(:,i))/deltaT;                             % Calculate velocity at discrete time step
%     xdot = [xdot_linear; omega];
%     J = self.robot2.model.jacob0(qMatrix(i,:));            % Get the Jacobian at the current state
%     J = J(1:6,:);                           % Take only first 2 rows
%     qdot = inv(J)*xdot;                             % Solve velocitities via RMRC
...
% end
            ...
            % self.qEndThree = self.robot2.model.ikcon(transl([-0.8 0.7800 1.45]) * rpy2tr(180,0,0,'deg');  % Calculate inverse kinematics to find joint configuration
            % 
            % 
            % % Generate the trajectory using LSPB for smooth motion
        ...
        function ReturnHome(self)
            ...
            % Animate the robot and gripper along the return home trajectory
            for j = 1:self.stepsThree
                self.robot2.model.animate(path5(j, :));  % Animate the robot joint states
